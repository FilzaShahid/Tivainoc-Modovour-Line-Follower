...
int main ( void ){
	...
	PortA_Init();
	while (1) {
	if ((GPIO_PORTA_DATA_R) == 0x44){ // Straight // On track
		GPTM_TA_MATCH_R  = 12000 ;
		GPTM_TA_MATCH_R2 = 12000 ;
}
	if ((GPIO_PORTA_DATA_R) == 0x0C){ //turn left
		GPTM_TA_MATCH_R  = 14000 ;
		GPTM_TA_MATCH_R2 = 10000 ;
}
	if ((GPIO_PORTA_DATA_R) == 0x60){ //turn right
		GPTM_TA_MATCH_R  = 10000 ;
		GPTM_TA_MATCH_R2 = 14000 ;
}
	if ((GPIO_PORTA_DATA_R) == 0x78){ //turn sharp right
		GPTM_TA_MATCH_R  = 10000 ;
		GPTM_TA_MATCH_R2 = 15999 ;
	}
	if ((GPIO_PORTA_DATA_R) == 0x3C){ //turn sharp left
		GPTM_TA_MATCH_R  = 15999 ;
		GPTM_TA_MATCH_R2 = 10000 ;
}
	if ((GPIO_PORTA_DATA_R) == 0x70){ //if slight right
		GPTM_TA_MATCH_R  = 15000 ;
		GPTM_TA_MATCH_R2 = 9000 ;
}
	if ((GPIO_PORTA_DATA_R) == 0x1C){ //if slight left
